...
async function processOneWebWatchProduct(
  ...
  try {
    const question = `Quel est le prix actuel (le plus récent possible) du produit "${produit}" (${unite}) à Dakar, Sénégal ? Cherche en respectant CET ORDRE DE PRIORITÉ, et arrête-toi dès que tu trouves un prix fiable à un niveau : (1) sources LOCALES sénégalaises (Auchan.sn en priorité, marchés de Dakar Tilène/Castors/Sandaga, autres sites/boutiques sénégalaises), (2) si rien trouvé, sources de la SOUS-RÉGION ouest-africaine (Côte d'Ivoire, Mali, autres pays CEDEAO), (3) en tout dernier recours seulement, sources étrangères hors sous-région. À chaque niveau, privilégie les sites web avec une vraie fiche produit/prix ; les pages Facebook (boutiques, Marketplace) ne sont à considérer qu'en complément ou si aucun site web n'a l'information. Réponds avec le prix en FCFA le plus précis trouvé et le nom du site/source où tu l'as trouvé. Si aucun prix fiable n'est trouvé, dis-le clairement.`
